...
def check_output_distribution(model, loader, device, epoch):
    ...
    with torch.no_grad():
        ...
        with autocast("cuda", dtype=torch.bfloat16):
            raw = model(images[:1])
            # Strip MetaTensor and handle all output formats
            raw_t = raw.as_tensor() if hasattr(raw, 'as_tensor') else raw
            if isinstance(raw_t, (list, tuple)):
                logits = raw_t[0]
            elif raw_t.ndim == 6:
                logits = raw_t.unbind(dim=0)[0]  # first supervision head
            else:
                logits = raw_t
...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        for step_idx, batch in enumerate(train_bar):
            ...
            # Strip MetaTensor wrapper — prevents silent shape/dtype issues downstream
            ...
            # ================= FIX 1: MASK SHAPE =================
            masks = masks.float()
            if masks.ndim == 4:   # (B, D, H, W) — add channel dim
                masks = masks.unsqueeze(1)
            # After NpyDataset label.npy is (1,D,H,W); DataLoader gives (B,1,D,H,W) — correct
            ...
            with autocast("cuda", dtype=torch.bfloat16):
                ...
                # MONAI wraps outputs in MetaTensor — .dim() and isinstance checks
                # can behave unexpectedly. Strip to plain tensor first.
                preds_t = preds.as_tensor() if hasattr(preds, 'as_tensor') else preds

                if isinstance(preds_t, (list, tuple)):
                    preds_list = [
                        (p.as_tensor() if hasattr(p, 'as_tensor') else p)
                        for p in preds_t
                    ]

                elif preds_t.ndim == 6:
                    # Stacked: (num_outputs, B, C, D, H, W) — unbind on dim 0
                    preds_list = list(preds_t.unbind(dim=0))

                elif preds_t.ndim == 5:
                    # Single output tensor — no deep supervision active
                    preds_list = [preds_t]

                else:
                    # Unexpected — log shape and treat as single output
                    print(f"\n  [WARN] Unexpected preds shape: {preds_t.shape}")
                    preds_list = [preds_t]

                # ================= FIX 4: LOSS =================
                # CRITICAL: must initialise as tensor on the correct device —
                # plain float 0.0 loses grad_fn on the first += operation.
                ...
                for i, pred in enumerate(preds_list):
                    ...
                    weight = DEEP_SUPR_WEIGHTS[i] if i < len(DEEP_SUPR_WEIGHTS) else 0.25
                    total_loss = total_loss + weight * loss_i  # += breaks autograd in some torch versions
